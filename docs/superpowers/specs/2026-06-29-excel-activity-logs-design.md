...
Нужна отдельная admin-only книга `.xlsm` для чтения и отчетов по логам. Ее не надо показывать обычным пользователям на вкладке `Аудиторам`.

Имя центральной книги:

```text
!Журнал активности Excel.xlsm
```

Книга должна лежать отдельно от пользовательского runtime. Рекомендуемое место рядом с хранилищем логов:

```text
NetworkAssetsRoot\_system\activity-logs\!Журнал активности Excel.xlsm
```
...
Файл должен открываться по паролю. Точное значение пароля задается отдельно перед реализацией или сборкой admin-книги.

При открытии admin-книга должна сама обновлять данные. Отдельная видимая кнопка `Обновить` не нужна.

При открытии она должна:
...
В центральной книге нужны admin-макросы и отчеты. Импорт запускается автоматически при открытии книги; видимая кнопка обновления не нужна.
...
- Точное значение пароля для открытия admin-книги.
- Точный набор скрытых admin-процедур для включения и отключения логгера во время поэтапного включения.

`Поэтапное включение` здесь означает безопасный запуск функции не сразу на всех компьютерах: сначала на dev-машине, потом на небольшой тестовой группе, потом шире. Для этого нужны скрытые admin-процедуры, чтобы можно было быстро включить или выключить логгер без пользовательских кнопок на Ribbon.

Heartbeat фиксируется на 5 минут.
