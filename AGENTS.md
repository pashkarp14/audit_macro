...
- Для company-release считать пайплайн фиксированным: сначала собрать staging-пакет из dev-кодовой базы, затем прогнать отдельный `company-package-policy-smoke-test.ps1`, потом поставить VBA-пароль, потом прогнать отдельный `vba-protection-smoke-test.ps1`, и только после этого обновлять `addin/release/company/На рабочий комп/` и `addin/release/company/Результаты проверок/`.
- Для company-release по умолчанию складывать в `addin/release/company/На рабочий комп/` только `AuditAddin.xlam` и `AuditAddin.version.txt`. Папки шаблонов РД и `rd-document-map.tsv` добавлять туда только если мы их явно меняли и релиз собирается с таким флагом.
